...
/**
 * Class Disciple_Tools_Homescreen_Apps_Magic_User_App
 */
class Disciple_Tools_Homescreen_Apps_Magic_Login_User_App extends DT_Magic_Url_Base {

    public $page_title = 'Create Contacts';
    public $page_description = 'Create a contacts';
    public $root = 'create_contacts'; // @todo define the root of the url {yoursite}/root/type/key/action
    public $type = 'create_contacts'; // @todo define the type
    ...
    public function footer_javascript(){
        ...
        <script>
            ...
            let jsObject = [<?php echo json_encode([
                ...
                'parts' => $this->parts,
                'translations' => [
                    'add' => __( 'Add Magic', 'disciple-tools-homescreen-apps' ),
                ],
            ]) ?>][0]
    ...
    public function update_record( WP_REST_Request $request ) {
        ...
        $args = [];
        if ( !is_user_logged_in() ){
            $global_name = apply_filters( 'dt_magic_link_global_name', __( 'Magic Link', 'disciple-tools-homescreen-apps' ) );
            $args['comment_author'] = sprintf( __( '%s Submission', 'disciple-tools-homescreen-apps' ), $global_name );
            wp_set_current_user( 0 );
            $current_user = wp_get_current_user();
            $current_user->add_cap( 'magic_link' );
            $current_user->display_name = sprintf( __( '%s Submission', 'disciple-tools-homescreen-apps' ), $global_name );
        }
...
Disciple_Tools_Homescreen_Apps_Magic_Login_User_App::instance();
